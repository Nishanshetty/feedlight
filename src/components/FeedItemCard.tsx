...
export default function FeedItemCard({ item, isRead, isStarred, isSelected, accentIndex, onActivate, onOpen, onToggleStar, elRef }: Props) {
  ...
  const preview = item.content ? stripHtml(item.content).slice(0, 200) : null;
    ...
    <li ref={elRef}
      ...
      {item.thumbnail_url ? (
        ...
        <div onClick={onActivate}
          className={`flex flex-col flex-1 p-5 cursor-pointer select-none ${isRead && !isSelected ? "opacity-50" : ""}`}>
          ...
          {preview && (
            <p className="text-xs font-body text-on-surface-variant line-clamp-3 leading-relaxed mb-6">{preview}</p>
          )}
